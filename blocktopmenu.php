...
class Blocktopmenu extends Module
{
    ...
    public function getContent()
    {
        ...
        // Ajout de jquery ui pour la fonction sortable
            ...
            foreach ($shops as $shop_id) {
                ...
                if (count($shops) == 1) {
                    if (is_array($items) && count($items)) {
                        // Utilisation de json au lieu d'un implode
                        $updated = Configuration::updateValue('MOD_BLOCKTOPMENU_ITEMS', json_encode($items), false, (int)$shop_group_id, (int)$shop_id);
                    } else {
    ...
    // Génère une liste au lieu d'un select
    protected function makeMenuOption()
    {
        ...
        foreach ($menu_item as $k => $item) {
            if (!$item) {
                continue;
            }
            $item = is_array($item) ? $item['name'] : $item->name;
            ...
            switch (substr($item, 0, strlen($values[1]))) {
                case 'CAT':
                    ...
                    if (Validate::isLoadedObject($category)) {
                        ...
                        $html .= '<li class="btn btn-default btn-block">'.$category->name.' <input type="hidden" name="items['.$k.'][name]" value="CAT'.$id.'"><ul></ul></li>'.PHP_EOL;
                    }
                    break;
                ...
                case 'PRD':
                    ...
                    if (Validate::isLoadedObject($product)) {
                        ...
                        $html .= '<li class="btn btn-default btn-block">'.$product->name.' <input type="hidden" name="items['.$k.'][name]" value="PRD'.$id.'"><ul></ul></li>'.PHP_EOL;
                    }
                    break;
                ...
                case 'CMS':
                    ...
                    if (Validate::isLoadedObject($cms)) {
                        ...
                        $html .= '<li class="btn btn-default btn-block">'.$cms->meta_title.' <input type="hidden" name="items['.$k.'][name]" value="CMS'.$id.'"><ul></ul></li>'.PHP_EOL;
                    }
                    break;
                ...
                case 'CMS_CAT':
                    ...
                    if (Validate::isLoadedObject($category)) {
                        ...
                        $html .= '<li class="btn btn-default btn-block">'.$category->name.' <input type="hidden" name="items['.$k.'][name]" value="CMS_CAT'.$id.'"><ul></ul></li>'.PHP_EOL;
                    }
                    break;
                ...
                case 'ALLMAN':
                    ...
                    $html .= '<li class="btn btn-default btn-block">'.$this->l('All manufacturers').' <input type="hidden" name="items['.$k.'][name]" value="ALLMAN0"><ul></ul></li>'.PHP_EOL;
                    break;
                ...
                case 'MAN':
                    ...
                    if (Validate::isLoadedObject($manufacturer)) {
                        ...
                        $html .= '<li class="btn btn-default btn-block">'.$manufacturer->name.' <input type="hidden" name="items['.$k.'][name]" value="MAN'.$id.'"><ul></ul></li>'.PHP_EOL;
                    }
                    break;
                ...
                case 'ALLSUP':
                    ...
                    $html .= '<li class="btn btn-default btn-block">'.$this->l('All suppliers').' <input type="hidden" name="items['.$k.'][name]" value="ALLSUP0"><ul></ul></li>'.PHP_EOL;
                    break;
                ...
                case 'SUP':
                    ...
                    if (Validate::isLoadedObject($supplier)) {
                        ...
                        $html .= '<li class="btn btn-default btn-block">'.$supplier->name.' <input type="hidden" name="items['.$k.'][name]" value="SUP'.$id.'"><ul></ul></li>'.PHP_EOL;
                    }
                    break;
                ...
                case 'LNK':
                    ...
                    if (count($link)) {
                        ...
                        $html .= '<li class="btn btn-default btn-block">'.Tools::safeOutput($link[0]['label']).' <input type="text" name="items['.$k.'][name]" value="LNK'.(int)$link[0]['id_linksmenutop'].'"><ul></ul></li>'.PHP_EOL;
                    }
                    break;
                ...
                case 'SHOP':
                    ...
                    if (Validate::isLoadedObject($shop)) {
                        ...
                        $html .= '<li class="btn btn-default btn-block">'.$shop->name.' <input type="text" name="items['.$k.'][name]" value="SHOP'.(int)$id.'"><ul></ul></li>'.PHP_EOL;
     ...
     * @param string $item
     * @return array
     * Retourne un array avec le type et l'id de l'item
     */
    protected function showTypeAndId($item){
        preg_match($this->pattern, $item, $value);
        $a['id'] = (int)substr($item, strlen($value[1]), strlen($item));
        $a['type'] = substr($item, 0, strlen($value[1]));
        return $a;
    }
    protected function makeMenu()
    {
        ...
        foreach ($menu_items as $menu_item) {
            ...
            $typeAndID = $this->showTypeAndId($item);
            $id = $typeAndID['id'];
            // Ici, il faudrait créer un callback pour générer le lien en fonction de la catégorie... Qui serait appelée dans chaque case
            switch ($typeAndID['type']) {
                case 'CAT':
                    ...
                    if($children = $menu_item->children){
                        ...
                        foreach ($children as $child){
                            $tId = $this->showTypeAndId($child);
                            $this->_menu .= $tId['type'].' N° '.$tId['id'].'<br>';
                        }
                ...
                case 'PRD':
                    ...
                    if (!is_null($product->id)) {
                        $this->_menu .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($product->getLink()).'" title="'.$product->name.'">'.$product->name.'</a>'.PHP_EOL;
                    }
                    break;
                ...
                case 'CMS':
                    ...
                    if (count($cms)) {
                        $this->_menu .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($cms[0]['link']).'" title="'.Tools::safeOutput($cms[0]['meta_title']).'">'.Tools::safeOutput($cms[0]['meta_title']).'</a>'.PHP_EOL;
                    }
                    break;
                ...
                case 'MAN':
                    ...
                    if (!is_null($manufacturer->id)) {
                        ...
                        $link = new Link;
                        $this->_menu .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($link->getManufacturerLink((int)$id, $manufacturer->link_rewrite)).'" title="'.Tools::safeOutput($manufacturer->name).'">'.Tools::safeOutput($manufacturer->name).'</a>'.PHP_EOL;
                    }
                    break;
                ...
                case 'SUP':
                    ...
                    if (!is_null($supplier->id)) {
                        $link = new Link;
                        $this->_menu .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($link->getSupplierLink((int)$id, $supplier->link_rewrite)).'" title="'.$supplier->name.'">'.$supplier->name.'</a>'.PHP_EOL;
                    }
                    break;
                ...
                case 'SHOP':
                    ...
                    if (Validate::isLoadedObject($shop)) {
                        $link = new Link;
                        $this->_menu .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($shop->getBaseURL()).'" title="'.$shop->name.'">'.$shop->name.'</a>'.PHP_EOL;
                    }
                    break;
                case 'LNK':
                    $link = MenuTopLinks::get((int)$id, (int)$id_lang, (int)$id_shop);
                    if (count($link)) {
                        ...
                        $this->_menu .= '<li><a href="'.Tools::HtmlEntitiesUTF8($link[0]['link']).'"'.(($link[0]['new_window']) ? ' onclick="return !window.open(this.href);"': '').' title="'.Tools::safeOutput($link[0]['label']).'">'.Tools::safeOutput($link[0]['label']).'</a>'.PHP_EOL;
                    }
                    break;
            }
            $this->_menu .= '</li>';
